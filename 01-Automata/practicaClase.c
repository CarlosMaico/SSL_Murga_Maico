...
void resultado(FILE* output, const char* input, t_estado estado)
{
    fprintf(output, "%-20s%-15s", input, ""); // Imprime el contenido de entrada y deja espacio para el resultado
    switch (estado) {
...
void lexer(FILE* input, FILE* output) {
    ...
    char fragmento[100]; // Para almacenar fragmentos del texto
    int i = 0;
    while((c = fgetc(input)) != EOF){
        if(c != CENTINELA){
            fputc(c, output);
            fragmento[i++] = c; // Almacena el carácter en el fragmento
            estado = tabla_transiciones[estado][char_to_enum(c)];
        }
        else{
            fragmento[i] = '\0'; // Termina el fragmento con nulo
            resultado(output, fragmento, estado);
            estado = ESTADO_INICIAL;
            i = 0;
        }
    }
    fragmento[i] = '\0'; // Termina el fragmento final con nulo
    resultado(output, fragmento, estado);
}
